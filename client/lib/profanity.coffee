...
filter_profanity = (text) ->
	...
	text_result = text.replace(profane_re, (all) -> all.split('').map((e) -> '*').join(''))
	if check_profanity text_result.replace(/[^\w ]/g, '')
		return '****'
	return text_result
