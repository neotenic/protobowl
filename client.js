...
var actionMode, addAnnotation, avg, changeQuestion, chatAnnotation, createBundle, cumsum, generateName, last_question, latency_log, public_name, renderPartial, renderState, renderTimer, setActionMode, sock, stdev, sync, sync_offset, sync_offsets, testLatency, time, userSpan, users,
...
$('.buzzbtn').click(function() {
  return sock.emit('buzz', 'yay', function(data) {
    if (data === 'http://www.whosawesome.com/') {
      setActionMode('guess');
      return $('.guess_input').data('input_session', Math.random().toString(36).slice(3)).val('').focus();
    } else {
