var actionMode, addAnnotation, addImportant, avg, changeQuestion, chatAnnotation, checkAlone, computeScore, compute_sync_offset, connected, createAlert, createBundle, createCategoryList, createStatSheet, cumsum, ding_sound, distribution, formatTime, guessAnnotation, handleCacheEvent, inner_socket, lastRendering, last_question, latency_log, mobileLayout, next, public_id, public_name, public_rooms, rate_limit_ceiling, rate_limit_check, recent_actions, removeSplash, renderCategoryItem, renderPartial, renderState, renderTimer, serverTime, setActionMode, skip, sock, stdev, sum, sync, sync_offset, sync_offsets, synchronize, teams, testLatency, time, updateTextAnnotations, updateTextPosition, userSpan, users,
...
renderCategoryItem = function(item) {
  var cat, percentage, s, val, value;
  s = 0;
  for (cat in distribution) {
    val = distribution[cat];
    s += val;
  }
...
$('.dist-picker .decrease').live('click', function(e) {
  var cat, item, s, val, _i, _len, _ref, _results;
  item = $(this).parents('.category-item');
  s = 0;
  for (cat in distribution) {
    val = distribution[cat];
  ...
  if (distribution[$(item).data('value')] > 0 && s > 1) {
    distribution[$(item).data('value')]--;
  }
  _ref = $('.custom-category .category-item');
  _results = [];
...
renderState = function() {
  var active_count, attr, attrs, badge, entities, idle_count, list, member, members, name, ranking, row, team, user, user_index, val, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
  if (sync.users) {
    teams = {};
    _ref = sync.users;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if ($('.teams').val()) {
      ...
      for (user_index = _n = 0, _len5 = _ref5.length; _n < _len5; user_index = ++_n) {
...
userSpan = function(user, global) {
  var prefix, scope, text, _ref, _ref1;
  prefix = '';
  ...
  if (global) {
    ...
    scope = $('<span>').addClass('user-' + user).addClass('username');
  }
...
$('.categories').change(function() {
  if ($('.categories').val() === 'custom') {
    createCategoryList();
    return $('.custom-category').slideDown();
  } else {
