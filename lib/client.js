...
var actionMode, addAnnotation, addImportant, avg, changeQuestion, chatAnnotation, computeScore, connected, createAlert, createBundle, createStatSheet, cumsum, formatTime, guessAnnotation, handleCacheEvent, inner_socket, lastRendering, last_question, latency_log, mobileLayout, next, public_id, public_name, removeSplash, renderPartial, renderState, renderTimer, serverTime, setActionMode, skip, sock, stdev, sum, sync, sync_offset, sync_offsets, synchronize, testLatency, time, updateTextAnnotations, updateTextPosition, userSpan, users,
...
userSpan = function(user) {
  ...
  if (user.slice(0, 2) === "__") {
    return $('<span>').addClass('user-' + user).text(user.slice(2));
  } else {
...
guessAnnotation = function(_arg) {
  var answer, correct, decision, done, early, id, interrupt, line, magic_number, marker, my_score, prompt, prompt_el, ruling, session, text, user;
  ...
  if (done) {
    ...
    if (correct === "prompt") {
      ...
      if (user === public_id) {
        ...
        magic_number = 1000;
        if ((magic_number - 15 <= my_score && my_score <= magic_number + 15)) {
          $('body').fireworks();
        }
      }
    } else {
...
$('.chatbtn').click(function() {
  setActionMode('chat');
  return $('.chat_input').data('input_session', Math.random().toString(36).slice(3)).data('begin_time', +(new Date)).val('').focus();
});
...
$('.buzzbtn').click(function() {
  ...
  return sock.emit('buzz', 'yay', function(status) {
    if (status === 'http://www.whosawesome.com/') {
      ...
      if (window._gaq) {
        return _gaq.push(['_trackEvent', 'Game', 'Buzz Accepted']);
      }
    } else {
      setActionMode('');
      if (window._gaq) {
        return _gaq.push(['_trackEvent', 'Game', 'Buzz Rejected']);
      }
    }
  });
});
...
$('.chat_form').submit(function(e) {
  ...
  if (window._gaq) {
    return _gaq.push(['_trackEvent', 'Chat', 'Typing Time', 'Posted Message', time]);
  }
});
...
$('body').keydown(function(e) {
  ...
  if (e.keyCode === 32) {
    ...
    return $('.pausebtn').click();
  } else if ((_ref3 = e.keyCode) === 47 || _ref3 === 111 || _ref3 === 191 || _ref3 === 67) {
    e.preventDefault();
    return $('.chatbtn').click();
  } else if ((_ref4 = e.keyCode) === 66) {
...
jQuery.fn.fireworks = function() {
  var i, _i, _results,
    _this = this;
  _results = [];
  for (i = _i = 0; _i < 8; i = ++_i) {
    _results.push(this.delay(Math.random() * 1000).queue(function() {
