...
guessAnnotation = function(_arg) {
  ...
  if ($('#' + id).length > 0) {
    ...
    ruling = $('<a>').addClass('label ruling').hide().attr('href', '#').attr('title', 'Click to Report').attr('rel', 'tooltip').data('placement', 'right');
    line.append(' ');
    line.append(ruling);
    addAnnotation(line);
  }
  ...
  if (done) {
    ...
    setTimeout(function() {
      return ruling.tooltip('show');
    }, 100);
    setTimeout(function() {
      return ruling.tooltip('hide');
    }, 1000);
    decision = "";
    if (correct === "prompt") {
        ...
        checkScoreUpdate = function() {
          ...
          if (magic_number > 0) {
            if (old_score < magic_number && updated_score >= magic_number) {
              ...
              return createAlert(ruling.parents('.bundle'), 'Congratulations', "You have over " + magic_number + " points! Here's some fireworks to set the mood.");
            }
          }
        };
        checkScoreUpdate();
