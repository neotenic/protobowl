...
var Cookies, Question, QuestionSchema, QuizRoom, app, categories, checkAnswer, clearInactive, countCache, countQuestions, crypto, cumsum, db, difficulties, error_question, express, fisher_yates, fs, getQuestion, http, io, journal, log, mongoose, names, parseCookie, port, reaped, restoreJournal, rooms, scheduledUpdate, sha1, syllables, updateCache, uptime_begin, url, watcher,
...
journal = function(room, sync) {
  ...
  req = http.request({
    ...
    return console.log("committed journal");
  });
  req.on('error', function() {
...
restoreJournal = function(callback) {
  var req;
  req = http.request({
    host: 'localhost',
    port: 15865,
    ...
    return res.on('end', function() {
      ...
      for (name in json) {
        data = json[name];
        if (!(name in rooms || !data.question)) {
...
io.sockets.on('connection', function(sock) {
  var config, cookie, headers, is_god, is_ninja, publicID, room, room_name, user,
    _this = this;
