...
try {
  ...
  current_queue = [];
  remote = {
    initialize_remote: function(cb) {
      return cb();
    },
    get_question: function(type, diff, cat, cb) {
      var index, temp_filtered;
      if (diff === current_difficulty && cat === current_category && current_queue.length > 0) {
        return cb(current_queue.shift());
      } else {
        ...
        current_queue = (function() {
          var _i, _len, _ref, _results;
          _ref = fisher_yates(temp_filtered.length);
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            index = _ref[_i];
            _results.push(temp_filtered[index]);
          }
          return _results;
        })();
        return cb(current_queue.shift());
    ...
    get_categories: function(type) {
      return listProps('category');
    },
    count_questions: function(type, diff, cat, cb) {
      return cb(filterQuestions(diff, cat).length);
    }
  };
}
...
QuizRoom = (function() {
  ...
  QuizRoom.prototype.get_question = function(cb) {
    var category,
      _this = this;
    category = (this.category === 'custom' ? this.distribution : this.category);
    return remote.get_question(this.type, this.difficulty, category, function(question) {
  ...
  QuizRoom.prototype.sync = function(level) {
    ...
    if (level >= 3) {
      data.difficulties = remote.get_difficulties(this.type);
      data.categories = remote.get_categories(this.type, this.difficulty);
...
io.sockets.on('connection', function(sock) {
  ...
  sock.on('distribution', function(data) {
    ...
    for (cat in _ref) {
      count = _ref[cat];
      if (room.distribution[cat] === 0 && count > 0) {
        room.emit('log', {
          user: publicID,
  ...
  sock.on('difficulty', function(data) {
    ...
    category = (this.category === 'custom' ? this.distribution : this.category);
    return remote.count_questions(room.type, room.difficulty, category, function(count) {
      room.emit('log', {
        user: publicID,
  ...
  sock.on('category', function(data) {
    ...
    if (data === "custom") {
      category = (this.category === 'custom' ? this.distribution : this.category);
      remote.count_questions(room.type, room.difficulty, category, function(count) {
        return room.emit('log', {
          user: publicID,
          verb: 'enabled a custom category distribution' + ' (' + count + ' questions)'
        });
      });
      return room.sync(3);
    } else {
      return remote.count_questions(room.type, room.difficulty, room.category, function(count) {
        return room.emit('log', {
          user: publicID,
...
restore_journal(function() {
  return remote.initialize_remote(function() {
