...
var Cookies, Question, QuestionSchema, QuizRoom, app, categories, checkAnswer, clearInactive, countCache, countQuestions, crypto, cumsum, db, difficulties, error_question, express, fisher_yates, fs, getQuestion, http, io, log, mongoose, parseCookie, port, reaped, rooms, scheduledUpdate, sha1, syllables, updateCache, uptime_begin, watcher,
    ...
    if (this.attempt.done) {
      ...
      if (this.attempt.correct) {
        ...
        for (id in _ref1) {
          ...
          pool = 0;
          if (user.sockets.length > 0 && new Date - user.last_action < 1000 * 60 * 10) {
            if (this.max_buzz !== null && user.times_buzzed >= this.max_buzz) {
              buzzed++;
            }
            pool++;
          }
